...
import {Lotto6Of49Ticket} from "../../domain/lotto-6-of-49-ticket";
import {Config} from "../config";

export class Pcso649GameExecutor extends PcsoGameExecutor<Lotto6Of49Ticket> {
    constructor(page: Page) {
        super('PCSO649G', page);
    }

    async createTicket(): Promise<Lotto6Of49Ticket> {
        if (Config.ticketType() === TicketType.LUCKY_PICK) {
            return Lotto6Of49Ticket.createLuckyPick();
        }
        if (Config.ticketType() === TicketType.MANUAL_PICK) {
            return Lotto6Of49Ticket.createManualPick(Config.ticketNumbers());
        }
        throw new Error(`Unhandled ticket type: ${Config.ticketType()}`);
    }

    async execute(): Promise<Lotto6Of49Ticket> {
    ...
    private async selectNumbers(ticket: Ticket): Promise<void> {
        ...
        if (balls.length !== 49) {
            throw new Error(`Invalid number of balls, expecting 49 but got ${balls.length}`);
        }
