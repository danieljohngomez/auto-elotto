...
import {Lotto6Of42Ticket} from "../../domain/lotto-6-of-42-ticket";
import {Config} from "../config";

export class Pcso642GameExecutor extends PcsoGameExecutor<Lotto6Of42Ticket> {
    constructor(page: Page) {
        super('PCSO642G', page);
    }

    async createTicket(): Promise<Lotto6Of42Ticket> {
        if (Config.ticketType() === TicketType.LUCKY_PICK) {
            return Lotto6Of42Ticket.createLuckyPick();
        }
        if (Config.ticketType() === TicketType.MANUAL_PICK) {
            return Lotto6Of42Ticket.createManualPick(Config.ticketNumbers());
        }
        throw new Error(`Unhandled ticket type: ${Config.ticketType()}`);
    }

    async execute(): Promise<Lotto6Of42Ticket> {
    ...
    private async selectNumbers(ticket: Ticket): Promise<void> {
        ...
        if (balls.length !== 42) {
            throw new Error(`Invalid number of balls, expecting 42 but got ${balls.length}`);
        }
