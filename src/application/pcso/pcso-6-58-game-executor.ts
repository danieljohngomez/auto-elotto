...
import {Lotto6Of58Ticket} from "../../domain/lotto-6-of-58-ticket";
import {Config} from "../config";

export class Pcso658GameExecutor extends PcsoGameExecutor<Lotto6Of58Ticket> {
    constructor(page: Page) {
        super('PCSO658G', page);
    }

    async createTicket(): Promise<Lotto6Of58Ticket> {
        if (Config.ticketType() === TicketType.LUCKY_PICK) {
            return Lotto6Of58Ticket.createLuckyPick();
        }
        if (Config.ticketType() === TicketType.MANUAL_PICK) {
            return Lotto6Of58Ticket.createManualPick(Config.ticketNumbers());
        }
        throw new Error(`Unhandled ticket type: ${Config.ticketType()}`);
    }

    async execute(): Promise<Lotto6Of58Ticket> {
    ...
    private async selectGame(): Promise<void> {
        ...
        const play658 = await this.page.waitForSelector(`img[alt="${this.gameId}"]`);
        await play658.click();
    }
    ...
    private async selectNumbers(ticket: Ticket): Promise<void> {
        ...
        if (balls.length !== 58) {
            throw new Error(`Invalid number of balls, expecting 58 but got ${balls.length}`);
        }
